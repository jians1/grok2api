...
启动配置由 `config.yaml` 管理，字段说明见 [`config.example.yaml`](../config.example.yaml)。Provider、批量任务、路由、媒体容量、审计和客户端密钥默认限制由管理端设置页持久化并热加载。
...
可热加载的 Provider、批量任务并发、路由、审计、媒体和代理参数由管理端设置页维护；数据库驱动、监听地址、Redis 与加密密钥仍通过启动配置生效。
